...
countries = as.character(popRural$ratio_female$iso3c[popRural$ratio_female$iso3c %in% names(contact_all)]) # to reorder the countries by name (not ISO)
# to reorder the countries by name (not ISO)


pdf(paste0('plots/contactmatrices2021_rural.pdf'),height=21*cm,width=13.8*cm)
for(i in 1:length(countries)){
  
  COUNTRY = countries[i]
  country_name = (popRural$ratio_female$countryname[popRural$ratio_female$iso3c %in% COUNTRY])
  ...
  plotPopPyramidRural(COUNTRY = COUNTRY,INDEX = 'a')
  popViewport()
