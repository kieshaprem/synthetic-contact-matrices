...
source('compare_contact_surveys/codes/functions_plot.r')

load('compare_contact_surveys/input/contact_datasets.Rdata')
load('output/syntheticmatrices_2021/contact_others.rdata')
load('output/syntheticmatrices_2021/contact_all.rdata')
...
if(SAVEPLOT) png('compare_contact_surveys/plots/contacts_otherlocations.png',height=19,width=15,units='cm',res=1000,pointsize=10)
if(1)
{
  grid.newpage()
  pushViewport(plotViewport(c(2,3,0,0.5)))
  # grid.rect()
  # pushViewport(viewport(layout=grid.layout(nrow=5,ncol=5,width=c(rep(1,5)),height=c(rep(1,6)))))
...
if(SAVEPLOT) png('plots/Fig 1.png',height=19,width=15,units='cm',res=1000,pointsize=10)
if(1)
{
  grid.newpage()
  pushViewport(plotViewport(c(2,3,0,0.5)))
